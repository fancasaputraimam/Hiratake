...
if [ "${PASANG_OPENWA:-0}" = "1" ]; then
  ...
  echo  ""
  kuning "  LANGKAH SELANJUTNYA — semua lewat dashboard Hiratake, tanpa sentuh server:"
  echo  "   1. Buka dashboard OpenWA (http://IP-VPS:2785) → buat API Key → SALIN (hanya tampil sekali)"
  echo  "   2. Buat sesi (mis. 'hiratake') → Start → scan QR dengan WhatsApp"
  echo  "      ⚠ PAKAI NOMOR KHUSUS, jangan nomor pribadi/utama (risiko diblokir WA)"
  echo  "   3. Dashboard Hiratake → login owner → tab WhatsApp → sub-tab Konfigurasi"
  echo  "        URL Gateway  : http://127.0.0.1:2785"
  echo  "        Nama Sesi    : hiratake  (samakan dengan langkah 2)"
  echo  "        API Key      : tempel dari langkah 1"
  echo  "        Webhook Secret : teks acak buatan sendiri, min. 20 karakter"
  echo  "   4. Klik 'Simpan Semua Konfigurasi' → 'Daftarkan Webhook Otomatis' → centang 'Aktifkan integrasi'"
  echo  "   5. Panel 'Langkah Menghubungkan' di halaman itu memandu tiap langkah + tombol 'Uji Koneksi'."
  hijau ""
fi
